...
if "weight" not in par_dict.keys():
	par_dict["weight"] = torch.Tensor([1,16,10,8,6])
...
if "ISRUC1_dir" not in par_dict.keys():
	...
	par_dict["ISRUC1_dir"] = "/mnt/nfsData17/ZhaiMengjie/Datasets/ISRUC-1/" # 修改数据地址
if "ISRUC3_dir" not in par_dict.keys():
	...
	par_dict["ISRUC3_dir"] = "/mnt/nfsData17/ZhaiMengjie/Datasets/ISRUC-3/" # 修改数据地址
if "MASS_dir" not in par_dict.keys():
	...
	par_dict["MASS_dir"] = "/mnt/nfsData17/ZhaiMengjie/Datasets/MASS_SS3_3000_25C-Cz/" # 修改数据地址
...
if "SLEEPEDF153_dir" not in par_dict.keys():
	...
	par_dict["SLEEPEDF153_dir"] = "/mnt/nfsData17/ZhaiMengjie/Datasets/sleep-edf-153-3chs" # 修改数据地址
...
if "homepap_dir" not in par_dict.keys():
	...
	par_dict["homepap_dir"] = "/mnt/nfsData17/ZhaiMengjie/Datasets/homepap/polysomnography" # 修改数据地址
...
if "dataset" not in par_dict.keys():
	par_dict["dataset"] = ["ISRUC3"]# ["ISRUC1","ISRUC3","MASS","homepap"]# ["ISRUC1","ISRUC3","MASS","SLEEPEDF153", "homepap"] # 从{"ISRUC1","ISRUC3","MASS"}中选择。命令行修改格式为字符串，逗号隔开即可
...
if "taiyang_channel" not in par_dict.keys():
	par_dict["taiyang_channel"] = ['EEG C3-REF','EEG O1-REF','EOG1'] 
...
if "data_double" not in par_dict.keys():
	par_dict["data_double"] = False   # True # "sleep_stage_classification" or "sleep_apnea_detection" or "sleep_arousal_detection"
# get input param
opts, args = getopt(sys.argv[1:], '', ['data_dir=', 'save_dir=', 'seed=', 'gpu_device=', 'kfold_num=',"JXY_shhs1-300_dir=","JXY_out_dim=","task=","model_select=",
									   ...
									   'ISRUC1_channel=','ISRUC3_channel=','MASS_channel=','SHHS_channel=', 'SLEEPEDF153_channel=','taiyang_channel=','ccshs_channel=','cfs_channel=','yasa_feature=', 'testonly=','data_double='])
...
class WeightedKLDivLoss(nn.Module):
    def __init__(self, class_weights):
        super().__init__()
        self.class_weights = class_weights.view(1, 1, -1)  # reshape to broadcast over [B, T, C]

    def forward(self, input_logits, target_probs):
        """
        input_logits: [B, T, C] - model raw outputs
        target_probs: [B, T, C] - soft labels
        """
        input_logits = input_logits.permute(0,2,1)
        log_probs = F.log_softmax(input_logits, dim=-1)  # [B, T, C]
        kl_div = target_probs * (torch.log(target_probs + 1e-8) - log_probs)  # [B, T, C]
        weighted_kl = kl_div * self.class_weights.to(input_logits.device)  # apply weights

        return torch.mean(torch.sum(weighted_kl, dim=-1))  # sum over classes, mean over B and T


class Logger(object):
    def __init__(self, log_file_path):
        self.terminal = sys.stdout
        self.log = open(log_file_path, "a", buffering=1)  # buffering=1 代表行缓冲
...
if par_dict["testonly"] is 'False':
	# 创建一个新的保存目录
	par_dict["save_dir"] = os.path.join(par_dict["save_dir"], f'experiment_{timestamp}/')

	# 路径不存在则创建
	if not os.path.exists(par_dict["save_dir"]):
		os.makedirs(par_dict["save_dir"])
	log_file = os.path.join(par_dict["save_dir"], f"{timestamp}.txt")
	# 打开文件并写入信息
	sys.stdout = Logger(log_file)

	print(f"Saving results to: {par_dict['save_dir']}")
else:
	log_file = os.path.join(par_dict["save_dir"], f"{timestamp}.txt")
	# 打开文件并写入信息
	sys.stdout = Logger(log_file)
...
def train_fold(cur_fold, train_loader, val_loader):
    ...
    loss_func_train = WeightedKLDivLoss(class_weights=par_dict["weight"].cuda()).cuda()
    ...
    for epoch in range(par_dict["num_epochs"]):
        ...
        for i, (batch_x, batch_y) in enumerate(train_loader):# batch_x_yasa,
            ...
            loss = loss_func_train(y_pre, batch_y)  # target 是 soft label, shape: [batch_size, 5]
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
			...
			# 250707 by MJZ 标签平滑
            batch_y = torch.argmax(batch_y, dim=-1)  # shape: [B, T]
            if len(train_gt_y) == 0:
                train_gt_y = batch_y
                train_pre_y = y_pre
            else:
